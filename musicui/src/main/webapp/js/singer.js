$(function(){
	...
	var url = 'http://127.0.0.1:8080/userloadsingerbyid.do?singerId='+singerId;
	$.ajax({
		type:"get",
		url:url,
		async:true,
		success:function(data){
			console.log(data)
			$("#singer_img").attr("src",data.data[0].singerimg);
			$("#singername").html(data.data[0].singername);
			$("#address").html(data.data[0].address);
			$("#xingzuo").html(data.data[0].xingzuo);
			$("#language").html(data.data[0].slanguage);
			$("#birthday").html(data.data[0].birthday);
		},
		error:function(){
			alert("请检查你的网络的设置！")
		}
...
function loadsongsbysingerid(indexpage){
	var singerId = GetQueryString("singerId");
	var url = 'http://127.0.0.1:8080/userloadsongsbysingerid.do?singerId='+singerId+'&indexpage='+indexpage;
	$.ajax({
		type:"get",
		url:url,
		async:true,
